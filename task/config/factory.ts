...
import { marketConfigs, StableMarketConfig, VolatileMarketConfig } from '../../utils/markets'
import { liveLog } from '../../utils/misc'
import { MARKET_TYPE } from '../../utils/constant'
import { waitForTx } from '../../utils/contract'

deployerTask('config:factory:create-markets', 'create all defined markets', async (taskArgs, hre, deployer) => {
  const factory = await hre.ethers.getContractAt('MarketFactory', deployer.getAddress('MarketFactory'))
  ...
  for (const config of configs) {
    if (!config.address) {
      ...
      if (config.type === MARKET_TYPE.VOLATILE) {
        ...
        liveLog(`Volatile Market ${config.name} is deployed with ${deployedAddress} on tx ${receipt.transactionHash}`)
        ...
        const event = receipt.events?.filter((e) => e.event == 'CreateStableMarket')[0]
        // @ts-ignore
        const deployedAddress = event.args[0]
        liveLog(`Stable Market ${config.name} is deployed with ${deployedAddress} on tx ${receipt.transactionHash}`)
      } else {
