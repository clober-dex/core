...
contract EmptyHeapEmptyTreeTestClaimCancelOrderGasReport is Test {
    ...
    function testGasReport() public {
        ...
        orderBookUtils.claimOrder(true, PRICE_INDEX_BID, fullyFilledBidOrderId, gasReporter.EHET_FullyClaimBid);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.claimOrder(false, PRICE_INDEX_ASK, fullyFilledAskOrderId, gasReporter.EHET_FullyClaimAsk);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.claimOrder(true, PRICE_INDEX_BID, partiallyFilledBidOrderId, gasReporter.EHET_PartiallyClaimBid);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.claimOrder(false, PRICE_INDEX_ASK, partiallyFilledAskOrderId, gasReporter.EHET_PartiallyClaimAsk);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.cancelOrder(MAKER, true, PRICE_INDEX_BID, openBidOrderId, gasReporter.EHET_FullyCancelBid);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.cancelOrder(MAKER, false, PRICE_INDEX_ASK, openAskOrderId, gasReporter.EHET_FullyCancelAsk);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.cancelOrder(
            MAKER, true, PRICE_INDEX_BID, partiallyFilledBidOrderId, gasReporter.EHET_PartiallyCancelBid
        );

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.cancelOrder(
            MAKER, false, PRICE_INDEX_ASK, partiallyFilledAskOrderId, gasReporter.EHET_PartiallyCancelAsk
        );
    }
}
