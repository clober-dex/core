...
contract EmptyHeapDirtyTreeTestClaimCancelOrderGasReport is Test {
    ...
    function testGasReport() public {
        ...
        orderBookUtils.claimOrder(true, PRICE_INDEX_BID, fullyFilledBidOrderId, gasReporter.EHDT_FullyClaimBid);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.claimOrder(false, PRICE_INDEX_ASK, fullyFilledAskOrderId, gasReporter.EHDT_FullyClaimAsk);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.claimOrder(true, PRICE_INDEX_BID, partiallyFilledBidOrderId, gasReporter.EHDT_PartiallyClaimBid);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.claimOrder(false, PRICE_INDEX_ASK, partiallyFilledAskOrderId, gasReporter.EHDT_PartiallyClaimAsk);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.cancelOrder(MAKER, true, PRICE_INDEX_BID, openBidOrderId, gasReporter.EHDT_FullyCancelBid);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.cancelOrder(MAKER, false, PRICE_INDEX_ASK, openAskOrderId, gasReporter.EHDT_FullyCancelAsk);

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.cancelOrder(
            MAKER, true, PRICE_INDEX_BID, partiallyFilledBidOrderId, gasReporter.EHDT_PartiallyCancelBid
        );

        vm.revertTo(snapshotId);
        snapshotId = vm.snapshot();
        orderBookUtils.cancelOrder(
            MAKER, false, PRICE_INDEX_ASK, partiallyFilledAskOrderId, gasReporter.EHDT_PartiallyCancelAsk
        );
    }
}
